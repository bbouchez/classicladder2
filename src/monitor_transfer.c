...
//			strcpy( pFileTransForMaster->FileName, "azerty.txt" );
			...
			sprintf( pFileTransForMaster->FileName, "%s/classicladder_master_current_project.cl2prjz",TmpDirectoryRoot );
	...
	{
		if ( InfosGUI->TargetMonitor.TransferFileNum==0 )
			sprintf( pFileTransForMaster->FileName, "%s/classicladder_transfer_to_master.cl2prjz",TmpDirectoryRoot );
...
//			strcpy( FileTransfer[ MONITOR_SERIAL_SLAVE ].FileName, "azerty.txt" );
			// save current project compressed in the temp directory, before sending it...
			sprintf( FileTransfer[ MONITOR_SERIAL_SLAVE ].FileName, "%s/classicladder_slave_current_project.cl2prjz",TmpDirectoryRoot );
