...
void InitSequential( void )
{
	...
	for( NumStep=0; NumStep<NBR_STEPS; NumStep++ )
	{
		...
		Sequential->Step[ NumStep ].OffDrawCrossStep = -1;
		Sequential->Step[ NumStep ].OffDrawCrossStepOffsetY = 0;
	}
	...
	for( NumPage=0; NumPage<NBR_SEQUENTIAL_PAGES; NumPage++ )
		Sequential->PageComment[NumPage][0] = '\0';
}  // InitSequential
// --------------------------------
	...
	for( NumStep=0; NumStep<NBR_STEPS; NumStep++ )
	{
		...
		if ( Sequential->Step[ NumStep ].NumPage!=-1 )
		{
			if ( Sequential->Step[ NumStep ].Activated )
				Sequential->Step[ NumStep ].TimeActivated += GeneralParamsMirror./*InfosGene->GeneralParams.*/PeriodMilliSecsTaskLogic;
			else
				Sequential->Step[ NumStep ].TimeActivated = 0;
