...
//								Width+1*WidDiv3, 3*Height+1*HeiDiv3);
			...
			DrawPangoText( cr, x-Width/*+WidDiv4/2*/, y+HeiDiv2-1+Height, -1, -1, "LD" );
			/* input : count up */
			...
			DrawPangoText( cr, x-Width/*+WidDiv4/2*/, y+HeiDiv2-1+Height*2, -1, -1, "CU" );
			/* input : count down */
			...
			DrawPangoText( cr, x-Width/*+WidDiv4/2*/, y+HeiDiv2-1+Height*3, -1, -1, "CD" );
			/* output : empty */
			...
			DrawPangoText( cr, x+Width-WidDiv4,y+HeiDiv2-1+Height, -1, -1, "Q" );
			/* output : full */
...
//								Width+1*WidDiv3, Height+1*HeiDiv3);
			...
			my_cairo_draw_color_line( cr, pTheRung->Element[EleX-1][EleY].DynamicInput/*TimerIEC->Input*/ && DisplayColorState,
								x-Width,y+HeiDiv2, x-Width+WidDiv3,y+HeiDiv2 );
			DrawPangoText( cr, x-Width/*+WidDiv4/1.5*/, y+HeiDiv2-1, -1, -1, "IN" );
			/* output : done */
...
//IsTheCurrentRungOrInError: 1=current; 2=error
void DrawLeftRightBars( cairo_t * cr, int OffX, int PosiY, int BlockWidth, int BlockHeight, int HeaderLabelAndCommentHeight, int LeftRightBarsWidth, int IsTheCurrentRungOrInError )
{
	int LeftRightBarsHeight = HeaderLabelAndCommentHeight+BlockHeight*RUNG_HEIGHT;

	if (IsTheCurrentRungOrInError==1 )
	{
	...
	else if (IsTheCurrentRungOrInError==2 )
	{
		// red background for a rung in error !
...
void DrawRung( cairo_t * cr, int NumRungToDraw, char cRungInEdit, int OffX, int PosiY, int BlockWidth, int BlockHeight, int HeaderLabelAndCommentHeight, char DrawingOption )
{
	...
	for (y=0;y<RUNG_HEIGHT;y++)
	{
		for(x=0;x<RUNG_WIDTH;x++)
		{
			...
			if ( TypeEle==ELE_COUNTER || TypeEle==ELE_TIMER_IEC || TypeEle==ELE_REGISTER || TypeEle==ELE_COMPAR || TypeEle==ELE_OUTPUT_OPERATE )
...
//Cairo						0, 0, InfosGene->BlockWidth*RUNG_WIDTH+50, InfosGene->PageHeight+50);
	...
	{
		int ParamCurrentRungOrInError = 0;
		char cInEdit = FALSE;
		...
		if( !EditDatas.ModeEdit && ScanRung==InfosGene->RungInError )
			ParamCurrentRungOrInError = 2;
		else
			ParamCurrentRungOrInError = (ScanRung==InfosGene->CurrentRung)?1:0;
		DrawLeftRightBars( cr, 0, ScanY, InfosGene->BlockWidth, InfosGene->BlockHeight, InfosGene->HeaderLabelCommentHeight, LeftRightBarsWidth, ParamCurrentRungOrInError );
