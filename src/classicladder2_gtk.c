...
void CreateFileSelection(char * Prompt,int CreateFileSelectionType)
{
	...
	else
	{
		...
		FilterProjects = gtk_file_filter_new( );
		gtk_file_filter_set_name( FilterProjects, _("ClassicLadder2 projects") );
		// BEB : for ClassicLadder2, we use a different extension as file are not compatible with Classic Ladder
		gtk_file_filter_add_pattern (FilterProjects, "*.cl2prj");
        gtk_file_filter_add_pattern( FilterProjects, "*.cl2p" );
		gtk_file_filter_add_pattern( FilterProjects, "*.cl2prjz" );
		gtk_file_chooser_add_filter( GTK_FILE_CHOOSER(FileSelector), FilterProjects );
		gtk_file_chooser_set_filter( GTK_FILE_CHOOSER(FileSelector), FilterProjects );
	}
...
*/
	...
	if ( comments )
	{
		...
		strcat (comments, "https://github.com/bbouchez/classicladder2\n");
...
//							"copyright", "Copyright (C) " CL_RELEASE_COPYRIGHT_YEARS " Marc Le Douarain\nmarc . le - douarain /At\\ laposte \\DoT/ net" ,
							"copyright", "Copyright (c) Benoit BOUCHEZ for ClassicLadder2 specific work\nOriginal ClassicLadder work is (c) Marc LE DOUARAIN" ,
//							"logo", example_logo,
							"title", _("About ClassicLadder2"),
//							"website", "http://sites.google.com/site/classicladder" ,
							"comments", comments ,
					   NULL );
		free( comments );
	}
}  // DoActionAboutClassicLadder
// --------------------------------
...
void ExportSvgOrPngFile( char * FileToCreate, char GoForSvgExport )
{
	...
	if ( iCurrentLanguage==SECTION_IN_LADDER )
	{
		...
		DrawLeftRightBars( cr, 0, 0, BLOCK_WIDTH_DEF, BLOCK_HEIGHT_DEF, BLOCK_HEIGHT_DEF/2, LeftRightBarsWidth, 0 );
...
/* GTK SEARCH BOX ... */
/* =========================================================================*/
extern char *SearchTypesNames[NBR_CL_SEARCH_TYPES];
...
//ForGTK3, deprecated...	gtk_tooltips_set_tip( SearchEntryTooltip, SearchEntry, "Variable to search or block number", NULL );
	...
	for( Scan=0; Scan<NBR_CL_SEARCH_TYPES; Scan++ )
