...
#include "protocol_modbus_master.h"     // Added 0.10.000 for runtime compilation
...
void display_version (void)
{
	printf( CL_PRODUCT_NAME " V" CL_RELEASE_VER_STRING "\n" CL_RELEASE_DATE_STRING "\n"
	       "\n"
               "Copyright (C) " CL_RELEASE_COPYRIGHT_YEARS " Benoit BOUCHEZ\n"
	       "\n"
	       "ClassicLadder2 is based on ClassicLadder written by Marc LE DOUARAIN"
	       "\n"
	       "ClassicLadder2 comes with NO WARRANTY\n"
	       "to the extent permitted by law.\n"
	       "\n"
	       "You may redistribute copies of ClassicLadder2\n"
...
void HardwareActionsAfterProjectLoaded( void )
{
    ...
    // Added in 0.10.000 : if communication thread starts too quickly after serial port is configured, communication fails on Arduino Modbus RTU slave
    // By adding this delay, problem disappears
    // TODO : maybe define the timing value as a configuration parameter
    sleep (1);
...
int ClassicLadderMain( int argc, char *argv[] )
{
	...
	// Initialize CL2 extension library
#ifdef USE_WXCL2_EXTENSIONS
	LoadCL2Ext();
#endif
...
#ifdef USE_WXCL2_EXTENSIONS
				// Test the wxCL2 extensions library
                //CL2ShowConfiguration(InfosGene);
                //CL2ShowSections(&SectionArray[0], 0);
#endif // USE_WXCL2_EXTENSIONS
