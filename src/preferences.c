...
#ifdef GTK_INTERFACE
#include <gtk/gtk.h>
#include "classicladder2_gtk.h" // for macro MY_GTK_WIDGET_IS_VISIBLE()...
#endif
...
{
	FILE * File;
	File = fopen( "classicladder2_prefs", "rt" );
	if ( File )
	{
		fclose(File);
		strcpy( PrefsFile, "classicladder2_prefs" );
	}
	else
	{
		File = fopen( "/etc/classicladder2_prefs", "rt" );
		if ( File )
		{
			fclose(File);
			strcpy( PrefsFile, "/etc/classicladder2_prefs" );
			strcpy( SetsVarsFile, "/etc/classicladder2_set_vars_list.csv" );
			strcpy( LogEventsDataFile, "/etc/classicladder2_log_events.bin" );
		}
		else
		{
			...
			if ( path )
			{
				...
				if ( EndWithSlash )
				{
					sprintf( PrefsFile, "%s.classicladder2_prefs", path );
					sprintf( SetsVarsFile, "%s.classicladder2_set_vars_list.csv", path );
					sprintf( LogEventsDataFile, "%s.classicladder2_log_events.bin", path );
				}
				else
				{
					sprintf( PrefsFile, "%s/.classicladder2_prefs", path );
	...
	if ( SetsVarsFile[0]=='\0' )
		strcpy( SetsVarsFile, "classicladder2_set_vars_list.csv" );
	if ( LogEventsDataFile[0]=='\0' )
		strcpy( LogEventsDataFile, "classicladder2_log_events.bin" );
