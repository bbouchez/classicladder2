...
void VerifyDirectorySelected( char * TheFileProject, char * NewDir )
{
	...
	{
		if ( strcmp( &NewDir[ strlen( NewDir ) -5 ], ".cl2p" )!=0 && strcmp( &NewDir[ strlen( NewDir ) -7 ], ".cl2prj" )!=0
			&& strcmp( &NewDir[ strlen( NewDir ) -8 ], ".cl2prjz" )!=0 )
		{
...
//v0.9.20, no more necessary...		InitTempDir( );
	...
	if ( FileProject[0]!='\0' )
	{
		...
		if ( strcmp( &FileProject[ strlen( FileProject ) -5 ], ".cl2p" )==0 || strcmp( &FileProject[ strlen( FileProject ) -7 ], ".cl2prj" )==0
			|| strcmp( &FileProject[ strlen( FileProject ) -8 ], ".cl2prjz" )==0 )
		{
...
//v0.9.20, no more necessary...		InitTempDir( );
	...
	if ( strcmp( &FileProject[ strlen( FileProject ) -8 ], ".cl2prjz" )==0 )
	{
		CompressPrj = TRUE;
	}
	else
	{
		if ( strcmp( &FileProject[ strlen( FileProject ) -7 ], ".cl2prj" )!=0 )
		{
			if ( strcmp( &FileProject[ strlen( FileProject ) -5 ], ".cl2p" )==0 )
				FileProject[ strlen( FileProject ) -4 ] = '\0';
			strcat( FileProject, ".cl2prj" );
		}
	}
...
//v0.9.5	struct dirent *pEnt;
	...
	if ( pProjectFileZ || pProjectFile )
	{
		...
		if ( CompressedProject )
			gzputs( pProjectFileZ, "_FILES_CLASSICLADDER2\n" );
		else
			fputs( "_FILES_CLASSICLADDER2\n", pProjectFile );
...
//v0.9.5		}
		...
		if ( CompressedProject )
		{
			gzputs( pProjectFileZ, "_/FILES_CLASSICLADDER2\n" );
			gzclose( pProjectFileZ );
		}
		else
		{
			fputs( "_/FILES_CLASSICLADDER2\n", pProjectFile );
			fclose( pProjectFile );
		}
...
//		fgets( Buff, 300, pProjectFile );
		...
		if ( strncmp( Buff, "_FILES_CLASSICLADDER2", strlen( "_FILES_CLASSICLADDER2" ) )==0 )
		{
...
//////printf("%s(): gzgets lineS=%s\n", __FUNCTION__, Buff);
				...
				if ( strncmp( Buff, "_/FILES_CLASSICLADDER2", strlen("_/FILES_CLASSICLADDER2") )==0 )
				{
